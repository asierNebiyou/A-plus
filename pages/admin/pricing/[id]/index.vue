...
const { pricingPlans, createPlan, updatePlan, isLoading, fetchPlans } =
  usePricing();
const plans = ref([]);
...
onMounted(async () => {
  await fetchPlans();
  plans.value = pricingPlans.value;
  populateForm();
});

const populateForm = () => {
  selectedPlan.value = plans.value.find((plan) => plan._id === planId) || null;
...
const handleSubmit = async () => {
  try {
    ...
    await updatePlan(planId, { ...form.value, features });
